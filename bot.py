...
# ============ ИИ через KIE.ai ============
async def ask_kie(question: str) -> str:
    if not KIE_API_KEY:
    ...
    try:
        ...
        prompt = (
            ...
            "Помогай ученикам и родителям с вопросами об английском, школьной программе, "
            "учёбе и формате занятий. Если точной информации о расписании, цене или условиях "
            "нет в вопросе, не выдумывай её — предложи уточнить у преподавателя.\n\n"
        ...
        def call_kie() -> dict:
            ...
            try:
                ...
                raise RuntimeError(
                    f"KIE API HTTP {exc.code}: {error_body}"
                ) from exc
        ...
        return answer or "ИИ не прислал текстовый ответ. Попробуй задать вопрос ещё раз."
...
async def ask_ai_receive(message: Message, state: FSMContext):
    if message.from_user.id in STUB_AI_IDS:
        ...
        answer = await ask_kie(message.text)
        await message.answer(answer)
    # Оставляем состояние активным, чтобы пользователь мог задавать вопросы подряд.
